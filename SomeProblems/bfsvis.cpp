#include <iostream>
#include <queue>
using namespace  std;
bool a[1002][1002] , mark[1002][1002];
int b[1002][1002] , h[5][3]={{1,0},{0,-1},{0,1},{-1,0}};
queue <pair <int,int> > c;
int Check(pair <int,int> x , int n , int m)
{
	if(x.first<n and x.first>=0 and x.second>=0 and x.second<m)
	{
		//cerr << x.first <<  " %% " << x.second << endl;
		return 1;
	}
	else
		return 0;
}
void VE( pair<int,int> x , int n , int m)
{
	for(int i=0;i<4;i++)
	{
		pair<int,int> k=x;
		k.first+=h[i][0];
		k.second+=h[i][1];
		if(Check(k,n,m))
			b[k.first][k.second]++;
		
	}
}
void BFS(int n , int m)
{
	while(!c.empty())
	{
		pair <int,int> k=c.front();
		mark[k.first][k.second]=1;
		c.pop();
		VE(k,n,m);
		for(int i=0;i<4;i++)
		{
			pair<int,int> g=k;
			g.first+=h[i][0];
			g.second+=h[i][1];
			//cerr << " ------------ " << endl;
			if(Check(g,n,m))
			{
				//cerr << b[g.first][g.second] << " && " << endl;
				if(b[g.first][g.second]>=2)
					if(mark[g.first][g.second]==0)
					{
						//cerr << " !! " << endl;
						c.push(g);
						a[g.first][g.second]=1;
						mark[g.first][g.second]=1;
					}
			}
		}
	}
}
int main()
{
	int n , m;
	cin >> n >> m;
	for(int i=0;i<n;i++)
		for(int j=0;j<m;j++)
		{
			cin >> a[i][j];
			if(a[i][j]==1)
			{
				pair <int , int > l;
				l.first=i;
				l.second=j;
				c.push(l);
			}
		}
	BFS(n,m);
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<m;j++)
			cout << a[i][j] << " ";
		cout << endl;
	}
}
